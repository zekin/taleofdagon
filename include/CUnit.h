...
class CUnit : public CRenderable, public IUnit {
protected:
  CSpriteSheet* hair;
  Armor* armor;
  CSpriteSheet* shadow;
...
public: 
  ...
  CUnit(float x, float y, float z) : CRenderable(x,y,z),
...
CUnit(float x, float y, float z, XYZ skin_color, XYZ hair_color) : 
  ...
  virtual void registerToMap(IMap *map) {
    INFO(LOG) << "Registering unit with Map " << map->getID();
  }
