...
private:
  ...
  void setColor(char number) {
    switch(number) {
      ...
      case '7':  glColor3f(1.0,0.0,1.0); break;
...
public:
    ...
    void write_text(char* string, float x, float y, float size) {
      ...
      for (char* string_iterator=string, i=0; *string_iterator!=0; string_iterator++, i++) {
        ...
        glBegin(GL_QUADS);
          glTexCoord2f(0,0); glVertex3f(i*size+x,y,0);
          glTexCoord2f(1,0); glVertex3f(i*size+x+size,y,0);
          glTexCoord2f(1,1); glVertex3f(i*size+x+size,y+size*2,0);
          glTexCoord2f(0,1); glVertex3f(i*size+x,y+size*2,0);
        glEnd();
      }
     ...
     void write_text(char* string, float x, float y, float size, float width) {
      ...
      for (char* string_iterator=string, i=0; *string_iterator!=0; string_iterator++, i++) {
        ...
        glBindTexture(GL_TEXTURE_2D, font_texture->getTextureID(frameNumberForFontCharacter(*string_iterator)));
        glBegin(GL_QUADS);
          glTexCoord2f(0,0); glVertex3f(i*size+x,y+j*size*2,0);
          glTexCoord2f(1,0); glVertex3f(i*size+x+size,y+j*size*2,0);
          glTexCoord2f(1,1); glVertex3f(i*size+x+size,y+(j+1)*size*2,0);
          glTexCoord2f(0,1); glVertex3f(i*size+x,y+(j+1)*size*2,0);
        glEnd();
      }
      glColor3f(1.0,1.0,1.0); 
    }
