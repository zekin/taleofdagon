  ...
  virtual void notify(Event* e) {
   switch(e->type) {
      ...
      case EVENT_KEYPRESS_UP:
        if (e->a=='w')
          CEventManager::getInstance()->notify(Event(EVENT_CAMERA_MOVE_END, DIRECTION_NORTH));
        if (e->a=='s')
          CEventManager::getInstance()->notify(Event(EVENT_CAMERA_MOVE_END, DIRECTION_SOUTH));
        if (e->a=='a')
          CEventManager::getInstance()->notify(Event(EVENT_CAMERA_MOVE_END, DIRECTION_WEST));
        if (e->a=='d')
          CEventManager::getInstance()->notify(Event(EVENT_CAMERA_MOVE_END, DIRECTION_EAST));
        break;
